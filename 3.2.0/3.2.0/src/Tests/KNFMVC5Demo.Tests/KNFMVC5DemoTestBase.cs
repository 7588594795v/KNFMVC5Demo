...
namespace KNFMVC5Demo.Tests
{
    public abstract class KNFMVC5DemoTestBase : AbpIntegratedTestBase<KNFMVC5DemoTestModule>
    {
        ...
        protected void UsingDbContext(Action<KNFMVC5DemoDbContext> action)
        {
            UsingDbContext(AbpSession.TenantId, action);
        }

        protected Task UsingDbContextAsync(Func<KNFMVC5DemoDbContext, Task> action)
        {
            return UsingDbContextAsync(AbpSession.TenantId, action);
        }

        protected T UsingDbContext<T>(Func<KNFMVC5DemoDbContext, T> func)
        {
            return UsingDbContext(AbpSession.TenantId, func);
        }

        protected Task<T> UsingDbContextAsync<T>(Func<KNFMVC5DemoDbContext, Task<T>> func)
        {
            return UsingDbContextAsync(AbpSession.TenantId, func);
        }

        protected void UsingDbContext(int? tenantId, Action<KNFMVC5DemoDbContext> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<KNFMVC5DemoDbContext>())
                {
        ...
        protected async Task UsingDbContextAsync(int? tenantId, Func<KNFMVC5DemoDbContext, Task> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<KNFMVC5DemoDbContext>())
        ...
        protected T UsingDbContext<T>(int? tenantId, Func<KNFMVC5DemoDbContext, T> func)
        {
            T result;
        ...
        protected async Task<T> UsingDbContextAsync<T>(int? tenantId, Func<KNFMVC5DemoDbContext, Task<T>> func)
        {
            T result;
